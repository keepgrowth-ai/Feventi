-- 004 · T-11 … T-14 · AC-01 … AC-10
--
-- La función que decide quién se queda con el último cupo. Todo lo demás del
-- proyecto es lectura comparado con esto.
--
-- OJO: 0037 la reescribe para traducir el error del índice único del asiento a
-- un mensaje legible. Con ESTA versión, el perdedor de la carrera por un asiento
-- recibe «duplicate key value violates unique constraint», que no se le muestra
-- a un fan. La garantía es la misma en las dos; lo que cambia es el texto.
...
comment on function public.reserve_order(uuid, jsonb) is
  'AC-05: bloquea los price_tiers con for update EN ORDEN DE ID — sin ese orden, dos sesiones con los mismos dos tiers se abrazan en un deadlock.';
