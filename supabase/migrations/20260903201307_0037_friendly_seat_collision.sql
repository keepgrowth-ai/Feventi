-- Corrige 0033. Lo destapó AC-06.
--
-- La carrera por el asiento se contiene bien —1 éxito de 20 peticiones HTTP
-- simultáneas— pero el perdedor recibía el error crudo de Postgres:
--
--   duplicate key value violates unique constraint "order_items_seat_unique"
--
-- Eso no se le muestra a un fan. Y el spec es explícito: un mensaje dice QUÉ
-- pasó y QUÉ hacer. Aquí lo que hay que hacer es elegir otro asiento, porque
-- alguien acabó de quedarse con ese hace medio segundo.
--
-- Se captura `unique_violation` alrededor del insert de los ítems, y se traduce.
-- El índice sigue siendo el que decide: no se cambia la garantía, solo el texto.
...
begin
  ...
  for v_tier in
    select t.id, t.stock, t.reserved, t.sold, t.price_cents, t.zone_id,
           (select count(*) from _items it where it.tier_id = t.id) as asked
      from public.price_tiers t
     where t.id in (select distinct tier_id from _items)
  ...
  update public.price_tiers t
    ...
    from (select tier_id, count(*) as asked from _items group by tier_id) it
