...
| WARN | `auth_leaked_password_protection` | **Pendiente de activar en el panel**, no es SQL: Authentication → Policies → «Leaked password protection». No se puede tocar con las herramientas MCP. Sin esto, Supabase Auth no consulta HaveIBeenPwned y acepta contraseñas ya filtradas. Cerrar antes de tener usuarios reales. |
